...
export const TableFilter = ({
  ...
  table: Table<TableEntity.TableRes>;
}) => {
  const searchValue = String(
    table.getColumn("expand-column")?.getFilterValue() ?? "",
  );
    ...
    <>
      <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <p className="max-w-xl text-sm leading-6 text-slate-400">
          이름 검색, 상태 필터, 기본 행 추가와 함께 행 클릭 시 우측 수정 패널이
          열리도록 구성해서 실제 사용감을 빠르게 확인할 수 있습니다.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <p className="text-sm text-slate-400">
            Visible Rows{" "}
            <span className="font-semibold text-white">{rowCount}</span>
          </p>
          <button
            type="button"
